...
const Apply = (req, callback) => {
  connection.query(
    `INSERT INTO Applications (PersonID,ApplicationContent,Postid) values ('${
      req.freelancer
    }','${req.text}','${req.postid}');`,
...
const RetriveAppliers = async (req, callback) => {
  await connection.query(
    `SELECT * from Applications where postid=${req.postid}`,
    function(error, results, fields) {
      var appliers = [];

      var test = results.map((elem) => {
        connection.query(
          `SELECT * from Freelancers where id=${elem.PersonID}`,
